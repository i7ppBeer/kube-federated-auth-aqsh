...
Shell scripts 可透過 `values.yaml` 的 `aqsh.scripts` 設定，mount 至 `/scripts/`（mode 0755）：
...
若你使用 parent/wrapper chart 並由 wrapper 透過 `files/scripts/*.sh` 產生
`<release>-aqsh-scripts` ConfigMap，請額外啟用：
